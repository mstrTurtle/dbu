...
// 把MessageBlock，Blob Reader（一个Handler）和Connector接在一起
int
ACE_Blob::read (ACE_Message_Block *mb, size_t length, size_t offset)
{
  ...
  if (connector_.connect (brp, inet_addr_) == -1) // 我想这个属于同步的Connector?
...
// 把MessageBlock，Blob Writer（一个Handler）和Connector接在一起
int
