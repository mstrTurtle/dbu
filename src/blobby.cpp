...
// 这个DEMO演示的是Option，MessageBlock，Connector和Handler的使用方法。
// 总而言之，为了使用Connector，你应当写一个Handler。有共同点的Handler甚至可以
// 用类派生的方法弄个出来。
...
ACE_TMAIN (int argc, ACE_TCHAR *argv[])
{
  ...
  Options *options = Options::instance (); // Option是他自己弄的类。
  options->parse_args (argc, argv);

  // Explain what is going to happen
  if (options->debug_) // 调试模式打log，输出Option类当前hold的状态。
    ACE_DEBUG ((LM_DEBUG,
