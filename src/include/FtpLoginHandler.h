...
public:
    int open(void * = 0) override
    {
        ...
        if (peer().send_n(loginCmd_, ACE_OS::strlen(loginCmd_)) == -1) {
            ACE_ERROR_RETURN(
                    (LM_ERROR,
                     ACE_TEXT("(%P|%t) Failed to send USER command\n")),
                    -1);
        }
        ...
        if (peer().send_n(passCmd_, ACE_OS::strlen(passCmd_)) == -1) {
            ACE_ERROR_RETURN(
                    (LM_ERROR,
                     ACE_TEXT("(%P|%t) Failed to send PASS command\n")),
                    -1);
        }
    ...
    int handle_input(ACE_HANDLE = ACE_INVALID_HANDLE) override
    {
        ...
        if (bytesRead > 0) {
            buffer[bytesRead] = '\0';
            ACE_DEBUG(
                    (LM_DEBUG, ACE_TEXT("(%P|%t) Received response: %s\n"),
                     buffer));
        } else if (bytesRead == 0) {
            ACE_DEBUG(
                    (LM_DEBUG,
                     ACE_TEXT("(%P|%t) Connection closed by peer\n")));
            ACE_Reactor::instance()->remove_handler(
                    this, ACE_Event_Handler::READ_MASK);
            delete this;
        } else {
            ACE_ERROR_RETURN(
                    (LM_ERROR, ACE_TEXT("(%P|%t) Error in handle_input\n")),
                    -1);
        }
...
int ACE_TMAIN(int argc, ACE_TCHAR *argv[])
{
    ...
    if (connector.connect(handler, serverAddr) == -1) {
        ACE_ERROR_RETURN(
                (LM_ERROR,
                 ACE_TEXT("(%P|%t) Failed to connect to FTP server\n")),
                1);
    }
